...
function Calculator() {
	...
	self.eventHandlers = function() {
		$(".keys--num").on("click", function() {
			self.handlers.handleInputNumbers($(this));
			console.log(math_data);
		});
		$(".keys--operators").on("click", function() {
		...
		$(".keys--ac").on("click", function() {
			self.handlers.handleAC();
			console.log(math_data);
		});
		$(".keys--delete").on("click", function() {
			self.handlers.handleDelete();
			console.log(math_data);
		});
		$(".keys--period").on("click", function() {
			self.handlers.handlePeriod();
			console.log(math_data);
		});
		$(".keys--equal").on("click", function() {
	...
	self.handlers = {
		...
		handlePeriod: function() {
			...
			if (
				splitLastNumber[splitLastNumber.length - 1] === "." ||
				lastNumber === ""
			) {
				return;
			} else {
		...
		handleEqual: function() {
			...
			if (total === Infinity) {
				$(".calc--display-content").text("Error");
				math_data = [""];
			} else {
	...
	self.repeatSolution = function() {
		...
		var current_total = parseFloat($(".calc--display-content").text());
